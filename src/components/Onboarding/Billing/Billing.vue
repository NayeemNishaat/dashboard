<template>
  <div class="flex flex-col md:flex-row">
    <div
      ...
      <BillingLeft v-if="!planStore.loading" @ChangePlan="ChangePlan" :plans="planStore.plans" :plan="currentPlan" />
      <LoaderDots v-else />
    </div>
    <div
      ...
      <Control @previous="previous" @next="next" :store="store" :nextLoading="planStore.finishing"
...
const currentPlan = computed(() => planStore.plans.find((plan) => plan.selected === true) || {} as DisplayPlan);
const hasFinishedOnboarding = computed(() => mainStore.getters["onboarding/hasFinishedOnboarding"]);
const previous = () => {
  emit("previous");
};
const next = async () => {
...
onMounted(async () => {
  ...
  planStore.plans = res.available_plans
    ...
    .map((plan) => {
      let displayPlan: DisplayPlan = Object.assign(plan, { features: [] as Array<string>, selected: false });
