<template>
  <div class="md:h-[calc(100%-80px)] realive w-full flex justify-center items-center 2xl:pr-8 pt-8 md:!pt-5">
      ...
      <div class="w-full flex gap-6 xl:gap-3 2xl:gap-6 items-stretch">
        ...
        <div class="w-full xl:w-[250px] 2xl:w-[340px] xl:shrink-0 grid gap-3">
          <ul class="grid gap-2.5">
            <li class="cursor-pointer" @click="ChangePlan(plan.plan_id)" v-for="plan in plans" :key="plan.plan_id"
              :class="plan.selected && 'group'">
              <div
                ...
                <div class="flex items-center gap-4">
                  <div>
                    <svg width="18" height="18" viewBox="0 0 18 18" fill="none" xmlns="http://www.w3.org/2000/svg">
                      <circle v-if="!plan.selected" cx="9" cy="9" r="8.5" stroke="#CCCCCC" />
                      <circle v-if="plan.selected" cx="9" cy="9" r="9" fill="white" />
                      <path v-if="plan.selected"
                  ...
                  <div class="space-y-1">
                    <h4 class="text-base font-bold group-even:text-white group-odd:text-white text-black">
                      {{ plan.name }}
                    </h4>
                    <p
                ...
                <div>
                  <p
                    ...
                    ${{ plan.price }}
                    <sub class="text-xs group-even:text-white group-odd:text-white text-brand-gray">/month</sub>
                  </p>
                </div>
              </div>
            </li>
...
import DataMenus from "@/components/Onboarding/DataMenus.vue";
import { PropType } from "vue";
import { DisplayPlan } from "./billing";
const props = defineProps({
  "plan": Object as PropType<DisplayPlan>,
