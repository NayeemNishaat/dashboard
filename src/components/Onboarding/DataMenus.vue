<template>
  <div class="pr-3 lg:p-4 2xl:p-8 bg-white xl:border border-brand-gray rounded-[16px] space-y-10">
    <h3 class="text-lg 2xl:text-xl font-semibold text-brand-gray-500">
      {{ plan.description }}
    </h3>
    <ul class="flex flex-col gap-4">
      <li v-for="(feature, i) in plan.features" :key="i"
        class="flex justify-between items-center gap-4 text-base text-black font-semibold">
        <span>{{ feature }}</span>
        <svg width="18" height="18" viewBox="0 0 18 18" fill="none" xmlns="http://www.w3.org/2000/svg">
          <circle cx="9" cy="9" r="9" fill="#FEB80A" />
          <path
...
import { DisplayPlan } from './Billing/billing';

const props = defineProps({
  plan: {
    type: Object as PropType<DisplayPlan>,
    required: true
  }
})
</script>
